...
import {
  getListProductUnReviewService,
  handleReviewProductBoughtService,
...
const Delivered = () => {
  ...
  const submitReview = async () => {
    if (rating === 0) {
      return setTextErr(
        language === keyMap.EN
          ? "Vui chọn số sao mà bạn muốn đánh giá"
          : "Vui chọn số sao mà bạn muốn đánh giá"
      );
    }
    ...
    <>
      <Modal isVisible={isModalVisible}>
        ...
        <View style={{ backgroundColor: "white" }}>
          <Text>
            <TextFormatted id="transaction.star" />:
          </Text>
          <View style={{ flexDirection: "row" }}>
            {[1, 2, 3, 4, 5].map((star) => (
              <TouchableOpacity key={star} onPress={() => handleRating(star)}>
                {star <= rating ? <Text>⭐</Text> : <Text>☆</Text>}
              </TouchableOpacity>
            ))}
          </View>
          <Text style={{ color: "red" }}>{textErr}</Text>
          <Text>
            <TextFormatted id="transaction.comments" />:
          </Text>
          <TextInput
          ...
          <TouchableOpacity style={{ borderWidth: 4 }} onPress={submitReview}>
            <Text>
              <TextFormatted id="transaction.submit" />
            </Text>
          </TouchableOpacity>
        </View>
      </Modal>
            ...
            <TouchableOpacity style={styles.infoProduct}>
              <View style={styles.nameShop}>
                ...
                <Text style={{ marginLeft: 20, fontWeight: "600" }}>
                  {language === keyMap.EN
                    ? `${item.productReviewData.productSupplierData?.firstName} ${item.productReviewData.productSupplierData?.lastName}`
                    : `${item.productReviewData.productSupplierData?.lastName} ${item.productReviewData.productSupplierData?.firstName}`}
                </Text>
              </View>
                  ...
                  <Image
                    ...
                    source={{
                      uri:
                        ...
                        item.productReviewData?.image[0],
                    }}
                  />
                </View>
                <View>
